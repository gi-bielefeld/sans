...
{   
	// no locking necessary, becuase only needed when bifrost graph is read, which is  not done in parallel
...
{    
	lock[bin].lock();
	bool res = kmer_table[bin][kmer].test(color);
	lock[bin].unlock();
	return res;
}
...
{    
	lock[bin].lock();
	bool res = kmer_tableAmino[bin][kmer].test(color);
	lock[bin].unlock();
	return res;
}
